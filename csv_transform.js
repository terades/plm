...
const expression = `{
  ...
  "resource": $trim($split(ProdTable.ProdRoute[0].ProdRouteJob[0].FIELD_ProdRouteJob_WrkCtrId, ",")[0]),
...
const file = process.argv[2] || 'test.csv';
...
if (logicalLines.length > 0 && logicalLines[0].startsWith('"timestamp"')) logicalLines.shift();
